...
export default function ServiceDetailPage({ data }: ServiceDetailPageProps) {
  ...
  const stages =
    ...
    data.focusPoints.map((point, index) => ({
      title: point,
      eyebrow: ["Focus", "Build", "Scale"][index] ?? "Deliver",
      description: data.summary,
      output: data.deliverables[index] ?? data.deliverables[0],
      points: [point]
    }));
      ...
      <section className="section-shell relative pt-36 pb-16 md:pt-44 md:pb-24">
        <motion.div initial="hidden" animate="show" variants={stagger} className="max-w-5xl">
          ...
          >
            ...
            Service {String(activeIndex + 1).padStart(2, "0")} / {data.eyebrow}
          </motion.div>
          <motion.h1
            variants={fadeUp}
          ...
          <motion.div variants={fadeUp} className="mt-8 flex flex-wrap gap-3">
            ...
            >
              <span>Explore service system</span>
      ...
      <section id="service-system" className="section-shell scroll-mt-28 pb-20 md:pb-28">
        ...
        <div className="grid gap-5">
          {stages.map((stage, index) => (
            ...
            >
              <span className="pointer-events-none absolute left-0 top-0 h-[2px] w-48 bg-gradient-to-r from-fuchsia-300/80 to-transparent transition duration-500 group-hover:from-white" />
              <span className="pointer-events-none absolute left-0 top-0 h-48 w-[2px] bg-gradient-to-b from-fuchsia-300/80 to-transparent transition duration-500 group-hover:from-white" />
              <span className="pointer-events-none absolute bottom-0 right-0 h-[2px] w-48 bg-gradient-to-l from-blue-300/80 to-transparent transition duration-500 group-hover:from-white" />
              <span className="pointer-events-none absolute bottom-0 right-0 h-48 w-[2px] bg-gradient-to-t from-blue-300/80 to-transparent transition duration-500 group-hover:from-white" />
              <div className="flex items-start justify-between gap-5 md:block">
                ...
                <div className="rounded-full border border-fuchsia-200/14 bg-fuchsia-300/[0.05] px-3 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.24em] text-fuchsia-100/62 md:hidden">
                  {stage.eyebrow}
                </div>
              </div>
              <div>
                <div className="hidden text-[0.68rem] font-semibold uppercase tracking-[0.24em] text-fuchsia-200/55 md:block">
                  {stage.eyebrow}
                </div>
                <h3 className="mt-2 text-2xl font-semibold leading-tight tracking-normal text-white md:text-3xl">
                  {stage.title}
                </h3>
        ...
        <div className="grid gap-8 rounded-[1.6rem] border border-fuchsia-200/16 bg-[linear-gradient(135deg,rgba(24,13,50,0.78),rgba(7,8,28,0.86))] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.24)] md:grid-cols-[0.8fr_1.2fr] md:p-8 lg:p-10">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-normal text-white md:text-5xl">
              Built for clarity, adoption, and measurable progress.
            </h2>
          </div>
        ...
        <div className="relative overflow-hidden rounded-[1.6rem] bg-[linear-gradient(105deg,#3f207f,#7547df,#4d238a)] p-7 shadow-[0_24px_90px_rgba(117,71,223,0.22)] md:p-10">
          ...
          <div className="relative max-w-4xl">
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-normal md:text-5xl">
              Move from capability design to confident execution.
            </h2>
            <Link
            ...
            >
              Explore {nextService.label} <ArrowIcon />
            </Link>
          </div>
        </div>
        <Link
